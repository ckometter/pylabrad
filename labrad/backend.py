...
class BaseConnection(object):
    ...
    def connect(self, host=C.MANAGER_HOST, port=None, timeout=C.TIMEOUT, password=None, use_tls=C.MANAGER_USE_TLS):
        self.host = host
        self.port = port
        self.ID = self._connect(password, timeout, use_tls=use_tls)
        self.connected = True
    ...
    def _connect(self, password=None, timeout=None, use_tls=C.MANAGER_USE_TLS):
...
try:
    ...
    class TwistedConnection(BaseConnection):
        def _connect(self, password, _timeout, use_tls):
            startReactor()
            self.cxn = self.call(getConnection, self.host, self.port, self.name, password, use_tls=use_tls).wait()
            return self.cxn.ID
...
class AsyncoreConnection(BaseConnection):
    def _connect(self, password, timeout, use_tls):
        if use_tls:
            raise Exception("TLS is not currently supported with the asyncore backend")
        self.connected = False
        self.serverCache = {}
        self.settingCache = {}
