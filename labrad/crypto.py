...
class SRP(object):
    def __init__(self, identity, password, group=SrpGroup1024, hash_class=hashlib.sha1):
        def generator():
            N, g = group.N, group.g
            ...
            # send identity and client A. get back salt and server B
            salt, B_bytes = yield identity, int_to_bytes(A)
            ...
            if u == 0:
                raise Exception("auth failed: incompatible server and client challenges")
        ...
        self.identity, self.A_bytes = self._generator.next()

    def process_server_challenge(self, salt, B_bytes):
        M1 = self._generator.send((salt, B_bytes))
        return M1

    def process_server_confirmation(self, M2_server):
        success = self._generator.send(M2_server)
        if not success:
